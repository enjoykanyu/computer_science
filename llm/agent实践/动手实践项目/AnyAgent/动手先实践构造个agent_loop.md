...
<font style="color:rgb(13, 13, 13);background-color:rgb(248, 248, 248);">在初筛检索得到的候选文档集上，使用更复杂、计算代价更高的模型，对文档与查询的相关性进行重新评估并排序的过程</font>
...
请根据参考资料给出准确、简洁的答案："""
    ...
    # 这里可以接入 OllamaLLM / ChatOpenAI 等
    return f"[Prompt 已构建，长度 {len(prompt)} 字符]\n\n{prompt}"
...
<img src="https://cdn.nlark.com/yuque/0/2026/png/21570810/1777304387728-0568f300-cd5d-4f8b-8dcf-a09702bab07a.png" width="502" title="" crop="0,0,1,1" id="u99c05cb2" class="ne-image">
...
```
这里采用的LangGraph直接拼接历史对话，让它"记住"之前的对话
<img src="https://cdn.nlark.com/yuque/0/2026/png/21570810/1779095401263-d11b6e1c-b6bb-47d2-88a5-0a0ebdb189c9.png" width="823" title="" crop="0,0,1,1" id="u5306d74d" class="ne-image">
让它记住它叫做ABC，下次问它会将历史的对话拼接，因此它的输入中有之前的输入"从现在开始你叫做ABC"这个指令
...
# A2A
### 概念
在A2A(Agent-to-Agent)协议出现之前,AlAgent之间的协作面临一个根本性问题:缺乏统一的标准通信协议。
不同团队、不同框架构建的Agent各自为政,无法实现跨平台、跨框架的互联互通。
这就像一个各国使节齐聚一堂,却发现每每个人只说自己国家的语言,没有翻译,根本无法沟通。

在A2A出现之前,业界主要依赖以下几种方式实现Agent间的协作。
- 第一种是硬编码集成,开发者需要为每一对Agent之间的通信编写专门的适配代码马,当Agent数量增长时,
集成工作量呈O(n^2)级别膨胀。 
- 第二种是自定义协议,每个团队从自己定义消息格式和传输方式,导致系统之间无法互操作。
- 第三种是通过共享记忆或消息队列等中间件间接通信,这种方式虽然解耦了Agent,但缺乏语义层面的约定,Agent之间难以理解彼此的能力和意图。
...
- 1，客户端Agent通过AgentCard解析器发现远程Agent的能力;   
- 2，所有通信都经过JSON-RPC2.0协议层标准化;
- 3，SSE流式传输支持长时间任任务的实时更新;
- 4，OAuth2/API Key确保跨组织通信的安全性。形成了"发现,认证,通信,执行"的完整闭环。
